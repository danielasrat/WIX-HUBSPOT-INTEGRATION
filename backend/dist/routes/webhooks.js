...
router.use(auth_1.requireWebhookSecret);
...
async function enqueueSyncJob(payload) {
    try {
        await (0, queue_1.getSyncQueue)().add("sync", payload, {
            attempts: 5,
...
router.post("/wix/form", async (req, res) => {
    ...
    await prisma_1.default.leadEvent.create({
        data: {
...
router.post("/hubspot", async (req, res) => {
    ...
    for (const event of events) {
        ...
        const connection = await (0, integration_service_1.getConnectionByPortalId)(portalId);
        if (!connection) {
            continue;
        }
